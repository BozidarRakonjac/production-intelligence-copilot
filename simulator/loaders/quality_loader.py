...
def load_quality(conn):
    ...
    cursor.execute("""
        SELECT udi, type, machine_id, machine_failure, failure_type, inserted_at
    ...
    machine_id_in_batch = None

    for i, row in enumerate(rows):
        udi, machine_type, machine_id, machine_failure, failure_type, inserted_at = row
        machine_id_in_batch = machine_id
            ...
            try:
                cursor.execute("""
                    INSERT INTO bronze.quality_inspections
                        (batch_id, machine_type, machine_id, inspected_at, sample_size,
                         defect_count, defect_type, description, pass_fail)
                    VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s)
                    ON CONFLICT (batch_id) DO NOTHING
                """, (
                    batch_number, machine_type, machine_id_in_batch, inserted_at,
                    50, defect_count, primary_defect, description, pass_fail
                ))
                inserted += 1
