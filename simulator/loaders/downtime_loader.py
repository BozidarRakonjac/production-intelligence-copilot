...
def load_downtime(conn):
    ...
    # Read all fault rows from telemetry
    cursor.execute("""
        SELECT udi, type, failure_type, inserted_at 
    ...
    for row in fault_rows:
        udi, machine_type, failure_type, started_at = row
        ...
        try:
            cursor.execute("""
                INSERT INTO bronze.downtime_logs
                    (udi, machine_type, reason_code, description, 
